...
// write this
  ...
  GPIO_PORTD_DIR_R &= ~0x01;    // 4) make PD0 input
  GPIO_PORTD_AFSEL_R |= 0x01;   // 5) enable alternate function on PD0
  GPIO_PORTD_DEN_R &= ~0x01;    // 6) disable digital I/O on PD0
  GPIO_PORTD_AMSEL_R |= 0x01;   // 7) enable analog functionality on PD0
